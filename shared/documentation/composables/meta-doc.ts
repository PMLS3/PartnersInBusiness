...
export async function useDocumentationMeta(
  ...
  const modelModifiers = computed(() => {
    const prop = meta.value?.meta?.props?.find(
      (prop: any) => prop.name === "modelModifiers"
    )
    //
    ...
    return (
      prop?.type.match(modifierRe)?.map((m: string) => m.replace(/"/g, "")) ??
      []
    )
  })
...
<<<<<<< HEAD
    ...
    const filteredProps = metaProps.filter(
=======
    console.log(meta.value?.meta, "-----------")
    console.log(meta.value?.meta?.props, "-----------")
    const props = meta.value?.meta?.props?.filter(
>>>>>>> 38b716d (start)
      (prop: any) =>
        !excludedProps.includes(prop.name) &&
        !prop.tags.some((tag: any) => tag.name === "default")
    )
...
>>>>>>> 38b716d (start)
  ...
  const exposed = computed(() =>
    meta.value?.meta?.exposed?.filter((item: any) => {
      ...
      const isModel =
        item.name === "modelValue" || item.name === "modelModifiers"
