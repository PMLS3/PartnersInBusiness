<script lang="ts" setup>
  ...
  const countriesFiltered = computed(() => {
    const source =
      possibleCountries.value.length > 0
  ...
  let parser:
    | typeof import("libphonenumber-js/max")["parsePhoneNumber"]
    | undefined
  ...
  function formatInput(value: string) {
    ...
    parsed = parser(value, {
      defaultCountry: (vcountry.value?.toUpperCase() ||
        currentCountry.value?.code) as CountryCode,
    })
    if (!parsed) {
    ...
    if (
      !vcountry.value ||
      (vcountry.value &&
        !possibleCountries.value.includes(vcountry.value as CountryCode))
    ) {
...
<template>
  <div class="relative flex w-full">
    ...
    <FormInput
      ...
      :rounded="props.rounded"
      :classes="{
        wrapper: 'relative grow [&_.nui-input-placeload]:ms-16',
        input: `${inputStart} !pe-8`,
        icon: '!w-16',
      }"
      v-bind="$attrs"
      @update:model-value="
